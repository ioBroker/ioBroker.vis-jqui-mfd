...
/**
 * The widgets reference their icons relative to the vis root (`widgets/jqui-mfd/img/...`), and the buttons are
 * drawn by a jQuery UI theme. This serves them: `widgets/...` from this repository, `lib/...` (the jQuery UI
 * themes) from the `www` folder of vis-2 if `VIS2_WWW` points to one. Without it the page uses a small stand-in of
 * the theme "redmond".
 */
function visFiles(): any {
    const www = process.env.VIS2_WWW;
    const roots: [string, string | undefined][] = [
        ['/widgets/', path.join(here, '..', '..', 'widgets')],
        ['/lib/', www ? path.join(www, 'lib') : undefined],
    ];
    return {
        name: 'vis-files',
