 ...
 * - `VIS2_WWW` should point to the `www` folder of vis-2 (e.g. `ioBroker.vis-2/packages/iobroker.vis-2/www`), then
 *   the buttons are drawn by the real jQuery UI themes of vis-2. Without it the page uses a stand-in of "redmond"
 *   and the images of the other themes are wrong.
 *
...
const SCALE = 2;
// The dialogs are placed in window coordinates, so the window has to be taller than the page - then no scrolling
// moves them away from their sections
const VIEWPORT = { width: 1900, height: 7000 };
...
const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'mfd-shots-'));
const chrome = spawn(
    findChrome(),
    [
        '--headless',
...
try {
    ...
    if (pageHeight > VIEWPORT.height) {
        console.warn(`The page (${pageHeight}px) is taller than the window - the dialogs may be misplaced`);
    }
