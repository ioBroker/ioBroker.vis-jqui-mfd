 ...
 * `widgets/jqui-mfd.html` - vis-2 replaces a widget type when a React widget declares the same id. And because
...
const LEGACY_HTML = path.join(HERE, '..', 'widgets', 'jqui-mfd.html');
 ...
 */
const DROPPED = {
    // the snapshot comes from the state; `url` was offered but never read
    tplValMfdCamSnapshot: ['url'],
    // the stream comes from the state; `url` was only read by the reload on wake-up, which then loaded the wrong URL
    tplValMfdCamMjpg: ['url'],
};
...
for (const name of WIDGETS) {
    ...
    if (info.visSet !== 'jqui-mfd') {
        console.log(`ERROR ${prefix}: visSet is "${info.visSet}", must be "jqui-mfd"`);
        problems++;
    }
